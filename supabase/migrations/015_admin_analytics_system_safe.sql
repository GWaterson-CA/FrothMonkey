...
-- Create analytics tracking tables for admin dashboard (only if they don't exist)
...
-- Create indexes for analytics tables (only if they don't exist)
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_page_views_created_at') THEN
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_page_views_path_created') THEN
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_listing_views_created_at') THEN
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_listing_views_listing_created') THEN
...
BEGIN
    ...
    -- Page views in time period (handle case where table doesn't exist)
    BEGIN
    ...
    -- Listing views in time period (handle case where table doesn't exist)
    BEGIN
...
-- Enable RLS on analytics tables (only if not already enabled)
DO $$ 
BEGIN
    IF NOT EXISTS (
...
DROP POLICY IF EXISTS "Admins can view all listing views" ON listing_views;
...
DROP POLICY IF EXISTS "Anyone can record listing views" ON listing_views;
CREATE POLICY "Anyone can record listing views" ON listing_views
    FOR INSERT WITH CHECK (true);

-- Add admin deletion policies for listings and users (DROP IF EXISTS first)
...
DROP POLICY IF EXISTS "Admins can delete any user profile" ON profiles;
