...
-- =====================================================
-- Issue: Migration 025_remove_duplicate_place_bid.sql overwrote place_bid
--        without preserving the process_auto_bids call from migration 038
-- 
-- This migration ensures place_bid ALWAYS calls process_auto_bids after
-- a manual bid is placed, so autobids trigger correctly.
-- =====================================================
...
BEGIN
    ...
    IF func_body NOT LIKE '%process_auto_bids%' THEN
        RAISE EXCEPTION 'FIX FAILED: place_bid function still does NOT call process_auto_bids!';
    ELSE
        RAISE NOTICE '✅ FIX APPLIED SUCCESSFULLY: place_bid now calls process_auto_bids';
    END IF;
END $$;
